...
void AppearanceInterfaceV1Private::bind_resource(Resource *resource)
{
    ...
    // If the config hasn't finished async initialization yet, skip the push;
    // values will be pushed when configInitializeSucceed fires (connected in
    // setupConfigConnections).
    auto *config = Helper::instance() ? Helper::instance()->config() : nullptr;
    if (!config || !isUserConfigInitialized(config))
        return;
    // Push model: send the current value of every setting immediately on bind.
...
{
    setupConfigConnections();
    // Skip the immediate push if the new config hasn't finished initializing;
    // configInitializeSucceed (connected in setupConfigConnections) will push
    // to all bound clients when initialization completes.
...
void AppearanceInterfaceV1::setupConfigConnections()
{
    ...
    // When async config initialization completes, push all values to bound
    // clients that bound before initialization was done.
    d->configConnections.append(
        connect(config, &TreelandUserConfig::configInitializeSucceed, this, [this] {
            for (const auto &resource : d->server->resourceMap())
                sendAll(resource->handle);
        }));
