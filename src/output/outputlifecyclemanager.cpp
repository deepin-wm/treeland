                                                    ...
                                                    Output *sourceOutput)
{
    ...
    for (auto *surface : surfaces) {
        ...
        m_configState->recordSurfaceBinding(surface, outputName, relativePos, state);
    }
}
...
void OutputLifecycleManager::restoreSurfaceBindings(Output *targetOutput)
{
    ...
    for (const auto &binding : validBindings) {
        ...
        if (savedState == SurfaceWrapper::State::Maximized
            ...
            Output *currentOutput = surface->ownsOutput();
            QRectF currentGeometry = currentOutput->geometry();

            qreal scaleX = currentGeometry.width() > 0
                ? targetGeometry.width() / currentGeometry.width() : 1.0;
            qreal scaleY = currentGeometry.height() > 0
                ? targetGeometry.height() / currentGeometry.height() : 1.0;

            QPointF posInCurrent = surface->position() - currentGeometry.center();
            QPointF newPos(targetGeometry.center().x() + posInCurrent.x() * scaleX,
                           targetGeometry.center().y() + posInCurrent.y() * scaleY);
...
QStringList OutputLifecycleManager::getActiveOutputNames() const
{
    QStringList names;
    if (!m_rootContainer)
        return names;
                                              ...
                                              const QList<SurfaceWrapper *> &allWorkspacesSurfaces)
{
    ...
    m_configState->cleanupStaleBindings(getActiveOutputNames());
}
